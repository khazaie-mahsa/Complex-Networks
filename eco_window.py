...
def plot_graph(G, size, a, b, gini):
    ...
    ax0.set_title(f"eco-window graph with size {size} and a = {a} and b = {b} and gini {gini}")
    ax0.set_axis_off()
...
for _ in range(0, simulation_size):
    ...
    a = 5
    b = 100
    gini, gintropy, h = eco_window_er(network_size, a, b)
    avg_gini += gini
