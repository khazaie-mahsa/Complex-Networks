...
from main import degree_to_np_array, calculate_gini, calculate_gintropy_max, \
    plot_lorenz_curve, create_connected_star_network
...
def communism_plus_plus(number_hub_nodes, number_leaf):
    ...
    plot_graph(G, nx.number_of_nodes(G), number_hub_nodes, number_leaf, gini)
    # draw_network(G)
...
for _ in range(0, simulation_size):
    G = 4  # Number of hubs
    L = 100  # Number of leaf nodes
    gini, gintropy, h = communism_plus_plus(G, L)
    avg_gini += gini
